...
import QrFlotante from "./QrFlotante";

const JUEGOS = [
  {
    emoji: "❌⭕",
    titulo: "Michi Nutricional",
    descripcion: "Tic-tac-toe con alimentos nutritivos vs chatarra. 1 vs 1 o contra la computadora.",
    href: "/nutri-kids/michi",
    bg: "bg-[#FFD93D]",
  ...
  },
  {
    ...
    descripcion: "Mueve la canasta y atrapa alimentos peruanos que nutren.",
    href: "/nutri-kids/atrapa-saludable",
    bg: "bg-[#B5EAD7]",
  ...
  },
  {
    emoji: "❓",
    titulo: "Trivia Nutri",
    descripcion: "3 niveles de dificultad sobre alimentos peruanos y hábitos saludables.",
  ...
  {
    emoji: "⚔️",
    titulo: "Nutri Batalla",
    descripcion: "Combate retro por turnos: Nutri-héroes peruanos contra malos hábitos.",
    href: "/nutri-kids/nutri-batalla",
    bg: "bg-[#AEE6FF]",
...
const PARA_PADRES = [
  {
    titulo: "Michi Nutricional",
    texto:
      "Refuerza el reconocimiento de categorías de alimentos: distinguir de un vistazo qué es un alimento que nutre y qué es comida de vez en cuando, sin usar un lenguaje de \"bueno vs malo\".",
  },
  {
    titulo: "Atrapa Saludable",
    texto:
      "Trabaja la toma de decisiones rápidas: elegir alimentos saludables peruanos bajo presión de tiempo, mientras aprende datos nutricionales reales de cada uno.",
  },
  {
    titulo: "Trivia Nutri",
    texto:
      "Construye conocimiento factual sobre nutrición y superalimentos peruanos (quinua, camu camu, tarwi y más), con explicaciones educativas en cada respuesta, correcta o no.",
  },
  {
    titulo: "Nutri Batalla",
    texto:
      "Conecta hábitos concretos (exceso de azúcar, sedentarismo, comida chatarra frecuente) con las elecciones nutricionales que los contrarrestan, en un formato de combate por turnos sin violencia real.",
  },
];

export default function NutriKidsIndexClient() {
  ...
  return (
    ...
    >
      ...
      {/* GRID DE JUEGOS */}
      <section className="max-w-4xl mx-auto px-4 pb-10 grid grid-cols-1 sm:grid-cols-2 gap-6">
        {JUEGOS.map((j) => (
          <div
            key={j.titulo}
      ...
      <section className="max-w-3xl mx-auto px-4 pb-10">
        <div className="bg-white/70 rounded-3xl p-6 md:p-8 border border-[#FFD93D]/40">
          <h2 className="font-playfair text-2xl text-[var(--verde-fuerte)] mb-4 text-center">Para padres</h2>
          <div className="flex flex-col gap-4">
            {PARA_PADRES.map((p) => (
              <div key={p.titulo}>
                <h3 className="font-semibold text-[#1a3d22] mb-1">{p.titulo}</h3>
                <p className="text-sm text-[#3a3a3a] leading-relaxed">{p.texto}</p>
              </div>
            ))}
          </div>
        </div>
